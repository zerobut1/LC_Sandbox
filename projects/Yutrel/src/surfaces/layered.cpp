...
public:
    ...
    {
        auto cos_theta_v = ite(abs(m_g) < 1e-3f,
                               1.0f - 2.0f * u.x,
                               -0.5f / m_g *
                                   (1.0f + sqr(m_g) -
                                    sqr((1.0f - sqr(m_g)) /
                                        (1.0f + m_g - 2.0f * m_g * u.x))));
...
public:
                            ...
                            TransportMode mode) const noexcept
    {
        ...
        $for(sample_index, m_ctx.samples)
        {
            $if(same)
            {
                ...
                auto wos = t_interface.sample(wo, rng.uniform(), make_float2(rng.uniform(), rng.uniform()),
                                              mode, ScatterFlags::Transmission);
                auto wis = t_interface.sample(wi, rng.uniform(), make_float2(rng.uniform(), rng.uniform()),
                                              reverse_mode(mode), ScatterFlags::Transmission);
                $if((wos.eval.pdf <= 0.0f) | (wis.eval.pdf <= 0.0f)) { $continue; };
                $if(is_delta_lobe(t_interface.lobe_flags()))
                {
                    pdf_sum += r_interface.evaluate(-wos.wi, -wis.wi, mode,
                                                    ScatterFlags::Reflection).pdf;
                }
                $else
                {
                    auto rs = r_interface.sample(-wos.wi, rng.uniform(),
                                                 make_float2(rng.uniform(), rng.uniform()),
                                                 mode, ScatterFlags::Reflection);
                    $if(rs.eval.pdf > 0.0f)
                    {
                        $if(is_delta_lobe(r_interface.lobe_flags()))
                        {
                            pdf_sum += t_interface.evaluate(-rs.wi, wi, mode,
                                                            ScatterFlags::Transmission).pdf;
                        }
                        $else
                        {
                            auto r_pdf = r_interface.evaluate(-wos.wi, -wis.wi, mode,
                                                              ScatterFlags::Reflection).pdf;
                            pdf_sum += power_heuristic(wis.eval.pdf, r_pdf) * r_pdf;
                            auto t_pdf = t_interface.evaluate(-rs.wi, wi, mode,
                                                              ScatterFlags::Transmission).pdf;
            ...
            $else
            {
                ...
                auto wos = to_interface.sample(wo, rng.uniform(), make_float2(rng.uniform(), rng.uniform()),
                                               mode);
                $if((wos.eval.pdf <= 0.0f) | (to_interface.to_local(wos.wi).z == 0.0f) |
                    ((wos.event & event_reflect) != 0u)) { $continue; };
                auto wis = ti_interface.sample(wi, rng.uniform(), make_float2(rng.uniform(), rng.uniform()),
                                               reverse_mode(mode));
                ...
                {
                    pdf_sum += ti_interface.evaluate(-wos.wi, wi, mode,
                                                     ScatterFlags::Transmission).pdf;
                }
                $elif(is_delta_lobe(ti_interface.lobe_flags()))
                {
                    pdf_sum += to_interface.evaluate(wo, -wis.wi, mode,
                                                     ScatterFlags::Transmission).pdf;
                }
                $else
                {
                    pdf_sum += 0.5f *
                               (to_interface.evaluate(wo, -wis.wi, mode,
                                                      ScatterFlags::Transmission).pdf +
                                ti_interface.evaluate(-wos.wi, wi, mode,
                                                      ScatterFlags::Transmission).pdf);
                };
            };
        };
                                               ...
                                               ScatterFlags requested_flags) const noexcept
    {
        ...
        $for(sample_index, m_ctx.samples)
        {
            auto wos = enter_interface.sample(wo, rng.uniform(), make_float2(rng.uniform(), rng.uniform()),
                                              mode, ScatterFlags::Transmission);
            auto wis = exit_interface.sample(wi, rng.uniform(), make_float2(rng.uniform(), rng.uniform()),
                                             reverse_mode(mode), ScatterFlags::Transmission);
            $if((wos.eval.pdf <= 0.0f) | (wis.eval.pdf <= 0.0f)) { $continue; };
            ...
            $for(depth, m_ctx.max_depth)
            {
                ...
                $else
                {
                    ...
                    $if((zp > 0.0f) & (zp < m_ctx.thickness))
                    {
                        ...
                        auto wt = ite(is_delta_lobe(exit_interface.lobe_flags()), 1.0f,
                                      power_heuristic(wis.eval.pdf, phase_pdf));
                        ...
                        {
                            auto exit = exit_interface.evaluate(-w, wi, mode,
                ...
                $if(z == exit_z)
                {
                    auto bs = exit_interface.sample(-w, rng.uniform(),
                                                    make_float2(rng.uniform(), rng.uniform()),
                                                    mode, ScatterFlags::Reflection);
                ...
                $else
                {
                    ...
                    {
                        auto nee = nonexit_interface.evaluate(-w, -wis.wi, mode,
                                                              ScatterFlags::Reflection);
                        auto wt = ite(is_delta_lobe(exit_interface.lobe_flags()), 1.0f,
                                      power_heuristic(wis.eval.pdf, nee.pdf));
                        f += beta * nee.f * wt * transmittance(m_ctx.thickness, wis_local) *
                             wis_f / wis.eval.pdf;
                    };
                    auto bs = nonexit_interface.sample(-w, rng.uniform(),
                                                       make_float2(rng.uniform(), rng.uniform()),
                                                       mode, ScatterFlags::Reflection);
                    $if(bs.eval.pdf <= 0.0f) { $break; };
                    beta *= bs.eval.f / bs.eval.pdf;
                    ...
                    $if(!is_delta_lobe(exit_interface.lobe_flags()))
                    {
                        auto exit = exit_interface.evaluate(-w, wi, mode,
                                                            ScatterFlags::Transmission);
                        auto wt = ite(is_delta_lobe(nonexit_interface.lobe_flags()), 1.0f,
                                      power_heuristic(bs.eval.pdf, exit.pdf));
        ...
        $if(bs.eval.pdf > 0.0f)
        {
            ...
            $else
            {
                ...
                $for(depth, m_ctx.max_depth)
                {
                    ...
                    auto next = interface.sample(-w, rng.uniform(),
                                                 make_float2(rng.uniform(), rng.uniform()), mode);
                    ...
                    $if((next.event & Surface::event_transmit) != 0u)
                    {
                        ...
                        auto event = ite(same_outer, Surface::event_reflect,
                                         ite(w_local.z > 0.0f,
                                             Surface::event_exit, Surface::event_enter));
                        result = Surface::Sample{
                            .eval = {.f = f, .pdf = path_pdf,
                                     .f_diffuse = SampledSpectrum{m_ctx.albedo.dimension()},
                                     .pdf_diffuse = 0.0f},
                            .wi = w,
                            .event = event,
                            .pdf_mis = 0.0f,
    ...
    Renderer& renderer, CommandBuffer& command_buffer) const noexcept
{
    return luisa::make_unique<Instance>(
        renderer, this, m_top->build(renderer, command_buffer),
...
{
    return luisa::format("Layered<{},{}>", m_top->closure_identifier(),
                         m_bottom->closure_identifier());
}
    ...
    SampledWavelengths& swl, Expr<float> time) const noexcept
{
    return luisa::make_unique<Closure>(
        renderer(), swl, time, m_top->create_closure(swl, time),
...
UInt Layered::Closure::lobe_flags() const noexcept
{
    ...
    auto type = ite(has_diffuse, Surface::lobe_diffuse,
                    ite(has_glossy, Surface::lobe_glossy, Surface::lobe_delta));
...
void LayeredSurfaceSpec::visit_dependencies(SpecDependencyVisitor& visitor) const noexcept
{
    ...
    if (m_params.thickness) { visitor.visit(*m_params.thickness); }
    if (m_params.albedo) { visitor.visit(*m_params.albedo); }
    if (m_params.g) { visitor.visit(*m_params.g); }
}
...
const Surface* LayeredSurfaceSpec::build(SceneBuilder& builder) const noexcept
{
    ...
    return builder.emplace<Surface, Layered>(
        builder.resolve(m_params.top), builder.resolve(m_params.bottom),
        resolve(m_params.thickness), resolve(m_params.albedo), resolve(m_params.g),
        m_params.max_depth, m_params.samples, m_params.two_sided);
}
} // namespace Yutrel
