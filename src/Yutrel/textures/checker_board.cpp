...
namespace Yutrel
{
    ...
    Float4 CheckerBoard::Instance::evaluate(const RTWeekend::HitRecord& rec) const noexcept
    {
        auto inv_scale = 1.0f / base<CheckerBoard>()->scale();
        auto position  = rec.position;
        ...
        return ite(is_even, m_even->evaluate(rec), m_odd->evaluate(rec));
    }
} // namespace Yutrel